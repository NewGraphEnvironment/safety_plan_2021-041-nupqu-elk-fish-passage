...
sites_cwf_raw <- sf::st_read('data/elk_cwf_priorities.geojson', quiet = T)
...
sites_elk_planning_raw <- sf::st_read(conn,
                                 query = "select modelled_crossing_id, my_priority, my_text from working.elk_flathead_planning_20210110") %>% 
...
tab_prep3 <- left_join(tab_prep2,
  ...
  mutate(source = case_when(modelled_crossing_id %in% sites_elk_planning ~ '2020_planning',
                            stream_crossing_id %in% sites_2020 ~ '2020_field',
                            aggregated_crossings_id %in% sites_cwf ~ 'cwf'))


table_sites <- left_join(tab_prep3, 
                         select(sites_elk_planning_raw, modelled_crossing_id, my_priority, my_text),
